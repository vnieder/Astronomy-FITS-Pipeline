...
import matplotlib
matplotlib.use('Agg') # to fix out of main error
...
def write_photometric_catalogues(light_frame_indicator:str, output_dir:str, threshold_multiplier:float) -> None:
    ...
    print("Matched files:", lights_processed)
    lights_processed.sort()
    radii = [3, 4, 5, 6, 8, 10, 12, 15, 20, 25]
    ...
    for i, file in enumerate(lights_processed):
        ...
        print(file)
        bkg = background(file)
        IRAFfind = IRAFStarFinder(fwhm=3.0, threshold=threshold_multiplier * bkg.background_rms_median,
...
def calculate_light_curves(output_directory:str, cat_indicator:str, light_frame_indicator:str, n_radii:int,
                           target_location:tuple, comparison_location:tuple, validation_location:tuple) -> tuple[list, list, list]:
...
def plot_light_curve(jd:list, lc_target_plot:list, lc_validation_plot:list, comparison_lc:list, norm_targ:float,
                     norm_vali:float, iaper:int, time_axis_range:list, directory_out:str, transit_name:str,
                     date:str, observer_name:str):
    ...
    plt.plot(time_axis_range, [.980, .980], 'b-', linewidth=0.5)
    plt.ylim([0.970, 1.01])
...
def to_csv(jd:list, target_lc:list, validation_lc:list, comparison_lc:list, norm_target:float,
           norm_vali:float, iaper:int, directory_out:str, filename:str):
...
def perform_photometry(light_frame_indicator:str, catalogue_indicator:str, output_dir:str,
                         threshold_multiplier:float, target_radius:int, target_location:tuple,
                         comparison_location:tuple, validation_location:tuple) -> tuple[list, list, list]:
    ...
    target_lc, comparison_lc, validation_lc = calculate_light_curves(output_dir, catalogue_indicator, light_frame_indicator,
                                                                     n_radii=10, target_location=target_location,
                                                                     comparison_location=comparison_location,
                                                                     validation_location=validation_location)
    return (target_lc, comparison_lc, validation_lc)

def plot(target_lc:list, comparison_lc:list, validation_lc:list, iaper:int, output_dir:str, transit_name:str,
...
# TODO: Fix hardcoded arguments
if __name__ == "__main__":
    write_photometric_catalogues("lrp", '/Users/vincentniedermayer/Desktop/test/test_output', 15.)
    calc_shifts_update_catalogues('/Users/vincentniedermayer/Desktop/test/test_output')
    target_lc, comparison_lc, validation_lc = calculate_light_curves('/Users/vincentniedermayer/Desktop/test/test_output', "cat", "lrp", n_radii=10,
                                                                      target_location=(2403.662364756954, 2018.2046627353204),
                                                                      comparison_location=(1756.1089736736467, 2364.1375110432928),
                                                                      validation_location=(1988.9627767690627, 1765.0694999895554))
    ...
    plot_light_curve(target_lc, lc_target_plot, lc_validation_plot, comparison_lc, norm_targ, norm_vali, 6,
                     time_axis_range, '/Users/vincentniedermayer/Desktop/test/test_output', "Qatar-5", "09/05/2024", "Vincent Niedermayer")
