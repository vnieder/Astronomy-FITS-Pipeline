...
def create_master_frames(directory, flip:bool):
    # we should not need to define another image collection
    ...
    # Load the bias frames as CCDData with appropriate units (e.g., "adu")
    bias_data = []
...
def calibrate_light_frames(directory:str, transit_name:str, master_bias:CCDData, master_flat:CCDData, target_coords_wcs:list, flip:bool) -> list:
    ...
    for light in lights:
        ...
        reduced.write(f"{directory}/test_output/{transit_name}_lrp_{counter}.fit", overwrite=True)
        counter += 1
    del files
    return light_frames
